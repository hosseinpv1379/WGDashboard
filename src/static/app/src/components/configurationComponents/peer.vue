	...
	<div class="card shadow-sm rounded-3 peerCard"
		...
		<div>
			...
			<div v-else class="border-0 card-header bg-transparent text-warning fw-bold" 
				...
				<LocaleText :t="Peer.quota_exceeded ? 'Data quota exhausted' : 'Access Restricted'"></LocaleText>
			</div>
		</div>
		<div class="card-body pt-1" style="font-size: 0.9rem">
			...
			<div class="d-flex"
				...
				<div v-if="Peer.quota_gb > 0" :class="{'d-flex gap-2 align-items-center' : dashboardStore.Configuration.Server.dashboard_peer_list_display === 'list'}">
					<small class="text-muted"><LocaleText t="Data Quota"></LocaleText></small>
					<small class="d-block" :class="{'text-danger fw-bold': Peer.quota_exceeded}">
						{{ (Peer.total_data + Peer.cumu_data).toFixed(2) }} / {{ Peer.quota_gb.toFixed(2) }} GB
					</small>
				</div>
