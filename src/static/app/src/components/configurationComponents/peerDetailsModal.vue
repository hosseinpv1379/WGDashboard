					...
					<div class="card-body px-4">
						...
						<div class="row mt-3 gy-2 gx-2 mb-2">
							...
							<div class="col-12 col-lg-3">
								<div class="card rounded-3 bg-transparent h-100">
									<div class="card-body d-flex flex-column justify-content-center">
										<p class="mb-0 text-muted"><small><LocaleText t="Data Quota"></LocaleText></small></p>
										<strong class="h4" :class="{'text-danger': selectedPeer.quota_exceeded}">
											{{ selectedPeer.quota_gb > 0 ? selectedPeer.quota_gb.toFixed(2) + ' GB' : GetLocale('Unlimited') }}
										</strong>
										<div v-if="selectedPeer.quota_gb > 0" class="progress mt-1" style="height: 6px">
											<div class="progress-bar" :class="selectedPeer.quota_exceeded ? 'bg-danger' : 'bg-info'"
											     :style="{width: Math.min(100, ((selectedPeer.total_data + selectedPeer.cumu_data) / selectedPeer.quota_gb) * 100) + '%'}"></div>
										</div>
									</div>
								</div>
							</div>
