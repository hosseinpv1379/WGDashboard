...
from .DatabaseConnection import ConnectionString, CreateEngine
...
class DashboardClients:
        ...
        with self.engine.connect() as conn:
            existingClient = conn.execute(
                self.dashboardClientsTable.select().where(
                    db.and_(
                        db.func.lower(self.dashboardClientsTable.c.Email) == Email.strip().lower(),
                        self.dashboardClientsTable.c.DeletedDate.is_(None),
                    )
                )
            ).mappings().fetchone()
    ...
    def SignUp(self, Email, Password, ConfirmPassword, Name=None) -> tuple[bool, str] or tuple[bool, None]:
        try:
        ...
        try:
            with self.engine.begin() as conn:
                ...
                if client.get("ClientGroup") == "Local":
                    ...
                    conn.execute(
                        self.dashboardOIDCClientsTable.update().values({
                            "DeletedDate": datetime.datetime.now(),
                        }).where(
            ...
            self.logger.log(Message=f"User {ClientID} soft deleted")
            self.__getClients()
        except Exception as e:
