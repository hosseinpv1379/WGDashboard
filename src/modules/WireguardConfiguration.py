...
class WireguardConfiguration:
    ...
    def __init__(self, DashboardConfig: DashboardConfig, 
        ...
        self.engine: sqlalchemy.Engine = CreateEngine("wgdashboard")
        ...
        with self.engine.begin() as conn:
            for i in listOfPublicKeys:
                ...
                if restrictedPeer is not None:
                    if restrictedPeer.get("quota_exceeded"):
                        conn.execute(
    ...
    def getPeersTransfer(self):
        ...
        self.enforceDataQuotas()

    def enforceDataQuotas(self) -> list[str]:
        """Restrict active peers whose persisted traffic reached their quota."""
        exceeded_peer_ids = []
        exceeded_at = datetime.now()
        with self.engine.begin() as conn:
            peers = conn.execute(
                self.peersTable.select().where(self.peersTable.c.quota_gb > 0)
            ).mappings().fetchall()
            for peer in peers:
                usage_gb = float(peer.get("total_data") or 0) + float(peer.get("cumu_data") or 0)
                if usage_gb >= float(peer.get("quota_gb") or 0):
                    exceeded_peer_ids.append(peer["id"])
                    conn.execute(
                        self.peersTable.update().values({
                            "quota_exceeded": 1,
                            "quota_exceeded_at": exceeded_at,
                        }).where(self.peersTable.c.id == peer["id"])
                    )

        if exceeded_peer_ids:
            status, message = self.restrictPeers(exceeded_peer_ids)
            if not status:
