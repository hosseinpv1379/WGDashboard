...
در اولین اجرا جدول‌های اشتراک تجاری و ستون‌های زمان انقضا به‌صورت خودکار ساخته
می‌شوند.
...
در منوی کناری باید گزینهٔ **Subscriptions & Nodes** نمایش داده شود.
...
1. وارد **Subscriptions & Nodes** شوید.
...
از منوی **Clients** یک Client بسازید. اشتراک تجاری حتماً به یک Client متصل
می‌شود.

### ۲. ساخت اشتراک

از **Subscriptions & Nodes** فرم **Create subscription** را تکمیل کنید:

| فیلد | توضیح |
| --- | --- |
| Client | صاحب اشتراک |
| Subscription name | نام نمایشی پلن |
| Data Quota | حجم مشترک تمام سرورها؛ `0` یعنی نامحدود |
| Expiration time | تاریخ و ساعت انقضا؛ خالی یعنی نامحدود |
| Maximum servers | بیشترین تعداد کانفیگ/Node این اشتراک |

لینک Subscription فقط هنگام ساخت یا Rotate نمایش داده می‌شود؛ آن را ذخیره کنید.

### ۳. ساخت کانفیگ روی Nodeها

1. فقط Nodeهای `online` قابل انتخاب هستند.
2. Nodeهای موردنظر را تیک بزنید.
3. **Provision selected nodes** را بزنید.
4. وضعیت Peer ابتدا `provisioning` و سپس `active` می‌شود.
